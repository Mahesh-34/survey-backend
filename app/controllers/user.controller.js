...
exports.create = (req, res) => {
  ...
  User.create(user)
    .then(data => {
      var user_data = {};
      user_data.token = createSession(data);
...
exports.findAllUsers = (req, res) => {
  User.findAll({ where: { is_user: true } })
    .then(data => {
      res.send(data);
    })
    .catch(err => {
...
exports.login = (req, res) => {
  ...
  User.findOne({ where: { email: useremail } })
    .then(data => {
      if(bcrypt.compareSync(userpassword, data.password)){
       var user_data = {};
       user_data.token = createSession(data, ip);
       console.log(user_data.token);
       user_data.user = data;       
        console.log(user_data);
        res.send(user_data);
 ...
 function createSession(user, ip){
  ...
  userdata.expires_at = new Date(Date.now() + 2 * (60 * 60 * 1000) ).getTime();
  console.log(userdata);
  var token_data = JSON.stringify(userdata);
...
function updateSession(data, ip){
  ...
  userdata.expires_at = new Date(Date.now() + 2 * (60 * 60 * 1000) ).getTime();
  var new_token_data = JSON.stringify(userdata);
  ...
  // Save User in the database
  UserSession.update(mew_session, {
...
exports.validateSession = (req, res) => {
  ...
  //console.log(token);
  UserSession.findOne({ where: { token : token } })
    .then(data => {
      User.findByPk(data.userId)
      .then(myuserdata => {
        if (myuserdata) {
           userdata = {};
          userdata.id = myuserdata.id;
          userdata.email = myuserdata.email;            
          userdata.email_verified = myuserdata.email_verified;    
    ...
    token_json = JSON.parse(token_data);
     ...
     console.log(data.ip);
      if(expriresAt >= date1.getTime() && ip == data.ip){ 
        var user_data = {};
        user_data.token = updateSession(data, ip);
        console.log(user_data.token); 
        console.log(userdata);
        user_data.user = userdata;
        user_data.user.ip = ip;
        user_data.user.expireAt = date1.getTime();   
      res.send(user_data);   
         } else {
  ...
  User.findOne({ where: { email : email } })
    .then(data => {
      ...
      const mailConfigurations = {  
        ...
        html: 'Please click on this <a href="' + process.env.SURVEY_BASE_URL + '/reset-password?code=' + code  + '">link</a> to reset password <br><br> <a href="' + process.env.SURVEY_BASE_URL + '/reset-password?code=' + code  + '">' + process.env.SURVEY_BASE_URL + '/reset-password?code=' + code  + '</a>'
    };
  ...
  User.findOne({ where: { email : email } })
    .then(data => {
      ...
      const mailConfigurations = {  
        ...
        html: 'Please click on this <a href="' + process.env.SURVEY_BASE_URL + '/verify-email?code=' + code  + '&email=' + email  +'">link</a> to verify your email <br><br> <a href="' + process.env.SURVEY_BASE_URL + '/verify-email?code=' + code  + '&email=' + email  + '">' + process.env.SURVEY_BASE_URL + '/verify-email?code=' + code  + '&email=' + email  + '</a>'
    };
    transport.sendMail(mailConfigurations, function(error, info){
      if (error) throw Error(error);
         console.log('Email Sent Successfully');
...
exports.verifyEmail = (req, res) => {
  ...
  console.log(code);
  if(code != null){
  User.findOne({ where: { verification_code : code } })
    .then(data => {
      if(!data){
      ...
      let status = User.update(new_code, {
...
exports.resetPassword = (req, res) => {
  ...
  console.log(code);
  if(code != null){
  User.findOne({ where: { forgotpassword_code : code } })
    .then(data => {
      if(!data){
        res.status(401).send({ message:"Invalid verification code"});
      } else {
